...
int main(int argc, char** argv) {
    ...
    for (unsigned int i = 0; i < devices.size(); i++) {
        ...
        // Create one command queue per CU pipeline
        ...
        if (err != CL_SUCCESS) {
            ...
            // Bind each kernel to its specific CU instance
            for (int cu = 0; cu < NUM_CUS; cu++) {
    ...
    // Per-CU buffers for time_series input and prediction output (double-buffered)
    ...
    // Shared model buffers (read-only, same data for both CUs but separate HBM banks)
    ...
    for (int cu = 0; cu < NUM_CUS; cu++) {
        ...
        // Model buffers (duplicated to separate HBM banks per CU)
    ...
    // Wait for all model data transfers to complete
    for (int cu = 0; cu < NUM_CUS; cu++) {
        q[cu].finish();
    }
    std::cout << "Model weights loaded to FPGA (" << NUM_CUS << " CU pipelines)" << std::endl;

    /*====================================================INFERENCE LOOP (overlapped 2-CU)==========================================*/

    int correct_predictions = 0;
    size_t num_samples = test_inputs.size();

    // Track which CU is processing which sample, and its completion events
    cl::Event event_k3[NUM_CUS];  // Track K3 completion (last kernel in pipeline)
    bool cu_busy[NUM_CUS] = {false, false};
    int cu_sample[NUM_CUS] = {-1, -1};  // Which sample each CU is processing

    auto total_start = std::chrono::high_resolution_clock::now();

    // Process all samples, dispatching to CUs round-robin
    size_t next_sample = 0;       // Next sample to dispatch
    size_t completed_samples = 0; // How many results we've collected

    // Results storage (indexed by sample)
    ...
    while (completed_samples < num_samples) {
        ...
        for (int cu = 0; cu < NUM_CUS && next_sample < num_samples; cu++) {
            ...
            // Copy time series data into this CU's buffer
            ...
            // Store events for per-kernel profiling
            // We need to keep references to k1/k2 events too
            // Store them in the results arrays after completion
            // Use a lambda-like approach: store events then extract timing after wait

            // Actually, we need to save these events. Let's use a simple struct approach.
            // For simplicity, we wait for this CU's queue to finish when we need to collect results.
        ...
        for (int cu = 0; cu < NUM_CUS; cu++) {
            ...
            // Wait for this CU's queue to finish
            q[cu].finish();
    ...
    std::cout << "\nMiniRocket Modular 2-CU FPGA inference complete!" << std::endl;
