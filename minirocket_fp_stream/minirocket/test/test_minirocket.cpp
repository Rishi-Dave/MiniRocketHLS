...
int main(int argc, char* argv[]) {
    ...
    for (int_t i = 0; i < (int)test_inputs.size(); i++) {
    ...
    int input_length = std::min((int)test_inputs.size(), MAX_TIME_SERIES_LENGTH);

    std::cout << "Running in quick test mode (limited to 100 samples)..." << std::endl;
    for (int test_idx = 0; test_idx < num_tests; test_idx++) {

        input_timeseries.write(test_inputs[test_idx]);
