...
\caption{FPGA accelerator for MiniRocket. Blue-shaded components support additional pooling operators for future work.}
...
We present two FPGA accelerator architectures, one for MiniRocket (Fig. \ref{fig:MiniMultiRocket_arch}) and the other for Hydra (Fig. \ref{fig:Hydra_arch})\footnote{
...
Both accelerators include Convolution Engines, which are functionally identical. The MiniRocket Convolution Engines (Fig. \ref{fig:MiniMultiRocket_arch}) are sized to the number of kernels and dilations, whereas Hydra's Group Convolution Engines (Fig. \ref{fig:Hydra_arch}) are sized to the number of kernels and dilations within a kernel group, and then replicated (two Convolution Engines per kernel group). MiniRocket's Pooling Engine computes the PPV feature exclusively, whereas Hydra's Group Pooling Engines compute the minimum hard count and maximum soft count histograms for each group. The regression classifier computes one dot product per class plus an $\argmax$.

\begin{figure*}[t]
\centering
\includegraphics[width=1\linewidth]{Figures/Hydra_Arch.pdf}
\caption{FPGA accelerator for Hydra. Network streaming (CMAC/UDP) was evaluated for MiniRocket only (Section \ref{sec:streaming_results}); Hydra results use the single-node configuration.}
...
Each padded convolution decomposes into a padding-dependent prefix, a shared interior, and a padding-dependent suffix. After $t_0$ arrives, the Convolution Engine rewrites the new convolution to reuse $T_{1..n-1} *_d\ \omega$:
\begin{equation}
\begin{aligned}
...
\begin{figure*}[tp]
    \centering
      ...
      \centering
      \includegraphics[width=\textwidth]{Figures/Convolution Engine-a.pdf}
      \caption{$0^4T_{1..23} *_1 \omega$}
      \label{fig:conv_eng_1}
    \end{subfigure}
    \hfill
    \begin{subfigure}[b]{0.48\textwidth}
        \centering
          \includegraphics[width=\textwidth]{Figures/Convolution Engine-b.pdf}
        \caption{$0^4T_{0..22} *_1 \omega$}
        \label{fig:conv_eng_2}
    \end{subfigure}
    \caption{Example of a convolution (dilation $d=1$) with dot-product reuse applied to a front-padded streaming time series $0^4T_{0..23}$ before (a) and after (b) a new datapoint is added to the window.}
...
\subsection{MiniRocket Floating-point Multiplication}
\label{sec:fp_mult}
...
In MiniRocket, each kernel $\omega$ consists of nine scalar values, six of which are $-1$ and three of which are $2$. 
Multiplication by $-1$ inverts the sign bit $s$ and multiplication by $2$ can be implemented by incrementing $e$ (we set the bias $b$ to $0$), with appropriate control logic for when $e$ overflows. %Multiplication by $3$, the constant favored in MiniRocket's software implementation, is somewhat more entailed; we omit details to conserve space. 

We propose a binary encoding for MiniRocket kernels: $\{0 \rightarrow -1, 1 \rightarrow 2\}$; this enables a compact floating-point multiplier design for encoded constants $-1$ and $2$. 
Consider floating-point operation $X \times Y$, where $X$ is any value, and $Y$ is either $-1$ or $2$; we use lower-case $y$ to represent the binary encoding of $Y$. Our proposed \textbf{'binary'} floating-point (\textbf{bFP}) multiplier is specified as:
\begin{equation}
    \langle XNOR(X.s, y), X.e + y, X.f\rangle.
    \label{eq:bFP}
\end{equation}
Convolution operators can compute dot products using the bFP multiplier, but must perform accumulation using standard floating-point accumulators. 
...
\subsection{Pooling and Classification Pipeline}
\label{sec:classification}

After feature extraction produces an $F$-dimensional feature vector (where $F = 84 \times |D| \times P$ for $|D|$ dilations and $P$ pooling operators), two additional pipeline stages complete the inference.

A \textbf{Scaler Kernel} normalizes each feature $f_i$ to zero mean and unit variance: $\hat{f}_i = (f_i - \mu_i) / \sigma_i$, where $\mu_i$ and $\sigma_i$ are precomputed from training data and stored in on-chip BRAM.

A \textbf{Classifier Kernel} computes ridge regression decision scores $s_c = \mathbf{w}_c^T \hat{\mathbf{f}} + b_c$ for each class $c$, selecting $\arg\max_c\, s_c$. The coefficient matrix $\mathbf{W} \in \mathbb{R}^{C \times F}$ and intercept vector $\mathbf{b} \in \mathbb{R}^C$ are precomputed on the host and transferred via HBM.

The Scaler and Classifier Kernels are connected to the Feature Extraction Kernel via AXI-Stream interfaces, forming a three-stage pipeline. Array partitioning distributes the feature map across multiple BRAM banks to enable concurrent write access by the 84 unrolled kernel loops. The pooling engine is pipelined rather than unrolled because the feature vector ($\sim$840 entries for MiniRocket) would require infeasible register resources if fully unrolled. For MiniRocket ($F = 840$), scaling and classification together consume $<$0.1\,ms per sample, making feature extraction the dominant bottleneck (85--99\% of total inference time).
