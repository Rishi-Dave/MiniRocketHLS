...
int main(int argc, char* argv[]) {
    ...
    hls::stream<data_t> input_timeseries;
    hls::stream<data_t> output_predictions;  
    ...
    int num_tests = std::min((int)test_inputs.size(), (csim) ? 1000 : 1); 

    std::cout << "\n" << std::string(60, '=') << std::endl;
    std::cout << "C++ MiniRocket Step-by-Step Comparison (Test Sample 1)" << std::endl;
    std::cout << std::string(60, '=') << std::endl;

    int_t window_size = (time_series_length < MAX_TIME_SERIES_LENGTH) ? time_series_length : (int_t) MAX_TIME_SERIES_LENGTH;
    ...
    if (csim & !full) {
        ...
        for (int test_idx = 0; test_idx < num_tests; test_idx++) {
            ...
            input_timeseries.write(test_inputs[0][test_idx]);
