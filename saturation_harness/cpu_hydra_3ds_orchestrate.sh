...
declare -A CFG_MODEL=(
  [InsectSound]="models/hydra_insectsound_model.json"
  [FruitFlies]="models/hydra_fruitflies_model.json"
  [MosquitoSound]="models/hydra_mosquitosound_model.json"
)

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
OUTDIR="$SCRIPT_DIR/runs/tp_2026-07-10_cpu_hydra"
LOGDIR="$OUTDIR/logs"
mkdir -p "$OUTDIR" "$LOGDIR"
CSV="$OUTDIR/cpu_hydra_3ds.csv"

log() { echo "[$(date '+%Y-%m-%d %H:%M:%S')] $*" | tee -a "$LOGDIR/cpu_hydra_3ds_orchestrate.log"; }

start_responder() {
  local dataset=$1
  local model=${CFG_MODEL[$dataset]}
  local stats="/tmp/cpu-hydra-${dataset}_stats.json"
  log "starting hydra responder on pc160 ${FPGA_DATA_IP}:${PORT} dataset=$dataset model=$model"
  ...
  timeout 10 $SSH "$PC160" "cd $REMOTE_DIR && setsid nohup python3 cpu_servers/$SCRIPT \
      ...
      --model-json $model \
      --variant cpu-hydra --dataset '$dataset' \
      --stats-file $stats \
      > /tmp/hydra_${dataset}_responder.log 2>&1 < /dev/null & echo \$!" > "$LOGDIR/hydra_${dataset}_responder.pid" 2>&1 || true
  sleep 1
  ...
  if [[ -z "$check" ]]; then
    ...
    $SSH "$PC160" "cat /tmp/hydra_${dataset}_responder.log" | tee -a "$LOGDIR/cpu_hydra_3ds_orchestrate.log"
    return 1
  fi
  log "  responder pid(s) on pc160: $check"
  echo "$check" > "$LOGDIR/hydra_${dataset}_responder.pid"
  return 0
}

stop_responder() {
  local pat="cpu_servers/[${SCRIPT:0:1}]${SCRIPT:1}.*--listen-port $PORT"
  log "stopping hydra responder on pc160"
...
run_rate_step() {
  ...
  local tp_out="/tmp/tp_cpu_hydra_${dataset}_${rate}"
  ...
  timeout $((DURATION_S + 20)) $SSH "$PC161" "python3 ~/throughput.py --fpga-ip $FPGA_DATA_IP --fpga-port $PORT \
      ...
      > "$LOGDIR/hydra_${dataset}_tp_${rate}.log" 2>&1
  local tp_rc=$?
  [ $tp_rc -ne 0 ] && log "  WARNING: throughput.py exited $tp_rc (see $LOGDIR/hydra_${dataset}_tp_${rate}.log)"
  ...
  python3 - "cpu-hydra" "$dataset" "$rate" "$sent" "$DURATION_S" "$processed" "$CSV" <<'PYEOF'
import sys
...
cat "$CSV" | tee -a "$LOGDIR/cpu_hydra_3ds_orchestrate.log"
